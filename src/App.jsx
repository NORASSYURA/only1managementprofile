...
function App() {
  ...
  const getOffDayStatus = (day) => {
    ...
    // Return the full request object so we can get the operator's name and status
...
<span>{op.name} ({op.email}) - Phone: {op.phoneNumber} - NRIC: {op.nric} - Job: {op.jobPosition} - Address: {op.homeAddress}</span>                            <div>
                ...
                {viewingDocs && (
                  <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.6)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000 }}>
                    <div style={{ backgroundColor: 'white', padding: '25px', borderRadius: '10px', maxWidth: '500px', width: '90%', maxHeight: '80vh', overflowY: 'auto', boxShadow: '0 10px 40px rgba(0,0,0,0.3)' }}>
                      ...
                      <button onClick={() => setViewingDocs(null)} style={{ marginTop: '15px', backgroundColor: '#6c757d', color: 'white', border: 'none', padding: '8px 20px', borderRadius: '5px', cursor: 'pointer', fontSize: '14px' }}>Close</button>
                    </div>
                  </div>
                )}
              </>
...
<strong>{req.operatorName}</strong> - Date: <strong>{req.requestedDate ? req.requestedDate.split('T')[0] : (req.date ? req.date : "Date Not Found")}</strong> - Status: <strong style={{ color: req.status === 'PENDING' ? 'orange' : req.status === 'APPROVED' ? 'green' : 'red' }}>{req.status}</strong>                        <br />Reason: {req.reason}
                        {req.status === 'PENDING' && (
                ...
                {myOffDayRequests.length > 0 ? (
                  <ul className="data-list">
                    {myOffDayRequests.map((req) => (
                      <li key={req.id}>Date: <strong>{req.requestedDate ? req.requestedDate.split('T')[0] : (req.date ? req.date : "Date Not Found")}</strong> - Status: <strong style={{ color: req.status === 'PENDING' ? 'orange' : req.status === 'APPROVED' ? 'green' : 'red' }}>{req.status}</strong></li>
                    ))}
                  </ul>
                ) : (
