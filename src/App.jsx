  ...
  if (user) {
    return (
      <div className="dashboard">
        ...
        <div className="main-content">
          {activePage === 'Overview' && (<><h1 className="dashboard-header">Welcome, {user.name}!</h1><div className="profile-card"><h3>Profile Details</h3><p><strong>Email:</strong> {user.email}</p><p><strong>Company ID:</strong> {user.companyId}</p><p><strong>Role:</strong> {user.role}</p><p><strong>Rate:</strong> ${user.rate ? user.rate : '0.00'}</p></div></>)}
          
          {activePage === 'My Profile' && (<><h1 className="dashboard-header">Welcome, {user.name}!</h1><div className="profile-card"><h3>My Profile Details</h3><p><strong>Email:</strong> {user.email}</p><p><strong>Company ID:</strong> {user.companyId}</p><p><strong>Role:</strong> {user.role}</p><p><strong>Rate:</strong> ${user.rate ? user.rate : '0.00'}</p></div><div className="data-section" style={{ marginTop: '20px' }}><h3>Update My Profile</h3><form onSubmit={handleProfileUpdate}><input type="text" placeholder="Home Address" value={homeAddress} onChange={(e) => setHomeAddress(e.target.value)} className="form-input" /><input type="text" placeholder="Phone Number" value={phoneNumber} onChange={(e) => setPhoneNumber(e.target.value)} className="form-input" /><input type="text" placeholder="NRIC" value={nric} onChange={(e) => setNric(e.target.value)} className="form-input" /><select value={jobPosition} onChange={(e) => setJobPosition(e.target.value)} className="form-input"><option value="">Select Job Position</option><option value="P103 Nightshift">P103 Nightshift</option><option value="Cc09b Dayshift">Cc09b Dayshift</option><option value="Changi T5 Dayshift">Changi T5 Dayshift</option><option value="Changi T5 Nightshift">Changi T5 Nightshift</option><option value="C12A/C12B">C12A/C12B</option><option value="Woodland ICA Dayshift">Woodland ICA Dayshift</option><option value="Xcmg - S.E Crane">Xcmg - S.E Crane</option><option value="HIROSE">HIROSE</option><option value="CR 106 Nightshift (Adhoc)">CR 106 Nightshift (Adhoc)</option><option value="Relieve Operator">Relieve Operator</option></select><button type="submit" className="action-btn">Update Profile</button></form></div><div className="data-section" style={{ marginTop: '20px' }}><h3>Upload Documents</h3><button onClick={() => setShowUploadForm(!showUploadForm)} className="action-btn">+ Upload File</button>{showUploadForm && (<div style={{ marginBottom: '15px' }}><input type="file" onChange={handleFileUpload} className="form-input" />{fileUrl && <button onClick={handleSaveDocument} className="action-btn" style={{ marginTop: '10px' }}>Save Document</button>}</div>)}{documents.length > 0 ? (<ul className="data-list">{documents.map((doc) => (<li key={doc.id}><a href={doc.fileUrl} target="_blank" rel="noopener noreferrer">{doc.fileName}</a></li>))}</ul>) : (<p>No documents uploaded yet.</p>)}</div></>)}

          {activePage === 'Jobs' && (<><h1 className="dashboard-header">Job Board</h1><div className="data-section">{jobs.length > 0 ? (<ul className="data-list">{jobs.map((job) => (<li key={job.id}><strong>{job.title}</strong> - {job.location} - ${job.rate}<br />{job.description}<br /><button className="action-btn">Sign Up</button></li>))}</ul>) : (<p>No jobs available.</p>)}</div></>)}
          
          {activePage === 'My Off Days' && (<><h1 className="dashboard-header">My Off Days</h1><div className="data-section"><button onClick={() => setShowOffDayForm(!showOffDayForm)} className="action-btn">+ Request Off Day</button>{showOffDayForm && (<div style={{ marginBottom: '15px' }}><input type="date" value={newOffDay.requestedDate} onChange={(e) => setNewOffDay({ ...newOffDay, requestedDate: e.target.value })} className="form-input" /><input placeholder="Reason" value={newOffDay.reason} onChange={(e) => setNewOffDay({ ...newOffDay, reason: e.target.value })} className="form-input" /><button onClick={handleCreateOffDay} className="action-btn">Submit Request</button></div>)}{myOffDayRequests.length > 0 ? (<ul className="data-list">{myOffDayRequests.map((req) => (<li key={req.id}>Date: <strong>{req.requestedDate ? req.requestedDate.split('T')[0] : (req.date ? req.date : "Date Not Found")}</strong> - Status: <strong style={{ color: req.status === 'PENDING' ? 'orange' : req.status === 'APPROVED' ? 'green' : 'red' }}>{req.status}</strong></li>))}</ul>) : (<p>No off day requests submitted.</p>)}</div></>)}
          
          {activePage === 'Relieve' && (<><h1 className="dashboard-header">Relieve Duty</h1><div className="data-section"><input type="date" value={newRelieve.date} onChange={(e) => setNewRelieve({ ...newRelieve, date: e.target.value })} className="form-input" /><select value={newRelieve.jobPosition} onChange={(e) => setNewRelieve({ ...newRelieve, jobPosition: e.target.value })} className="form-input"><option value="">Select Shift</option><option value="P103 Nightshift">P103 Nightshift</option><option value="Cc09b Dayshift">Cc09b Dayshift</option><option value="Changi T5 Dayshift">Changi T5 Dayshift</option><option value="Changi T5 Nightshift">Changi T5 Nightshift</option><option value="C12A/C12B">C12A/C12B</option><option value="Woodland ICA Dayshift">Woodland ICA Dayshift</option><option value="Xcmg - S.E Crane">Xcmg - S.E Crane</option><option value="HIROSE">HIROSE</option><option value="CR 106 Nightshift (Adhoc)">CR 106 Nightshift (Adhoc)</option></select><button onClick={handleCreateRelieve} className="action-btn">Submit Relieve</button></div><div className="data-section"><h3>My Relieve Duties</h3>{relieveRequests.filter(r => r.relieverId === user.id).length > 0 ? (<ul className="data-list">{relieveRequests.filter(r => r.relieverId === user.id).map((req) => (<li key={req.id}>{req.jobPosition} - {req.date}</li>))}</ul>) : (<p>No relieve duties assigned.</p>)}</div></>)}
          
          {activePage === 'Feedback' && (<><h1 className="dashboard-header">My Feedback</h1><div className="data-section"><button onClick={() => setShowFeedbackForm(!showFeedbackForm)} className="action-btn">+ Submit Feedback</button>{showFeedbackForm && (<div style={{ marginBottom: '15px' }}><input placeholder="Job Title" value={newFeedback.jobTitle} onChange={(e) => setNewFeedback({ ...newFeedback, jobTitle: e.target.value })} className="form-input" /><input placeholder="Comment" value={newFeedback.comment} onChange={(e) => setNewFeedback({ ...newFeedback, comment: e.target.value })} className="form-input" /><select value={newFeedback.rating} onChange={(e) => setNewFeedback({ ...newFeedback, rating: parseInt(e.target.value) })} className="form-input"><option value="5">5 Stars</option><option value="4">4 Stars</option><option value="3">3 Stars</option><option value="2">2 Stars</option><option value="1">1 Star</option></select><button onClick={handleCreateFeedback} className="action-btn">Submit Feedback</button></div>)}{myFeedback.length > 0 ? (<ul className="data-list">{myFeedback.map((fb) => (<li key={fb.id}><strong>{fb.jobTitle}</strong> - Rating: {fb.rating}/5<br />Comment: {fb.comment}</li>))}</ul>) : (<p>No feedback submitted yet.</p>)}</div></>)}
    ...
    <div className="login-container">
      <div className="login-box">
        ...
        <form onSubmit={isRegistering ? handleRegister : handleLogin}>
          {isRegistering && (<div className="form-group"><select value={role} onChange={(e) => setRole(e.target.value)} className="form-input"><option value="USER">User</option><option value="MANAGER">Manager</option><option value="ADMIN">Admin</option></select><input type="text" placeholder="Name" value={name} onChange={(e) => setName(e.target.value)} className="form-input" /></div>)}
          <div className="form-group"><input type="email" placeholder="Email" value={email} onChange={(e) => setEmail(e.target.value)} className="form-input" /></div>
          <div className="form-group"><input type="password" placeholder="Password" value={password} onChange={(e) => setPassword(e.target.value)} className="form-input" /></div>
          <button type="submit" className="login-btn">{isRegistering ? 'Sign Up' : 'Login'}</button>
        </form>

        <div style={{ textAlign: 'center', marginTop: '10px' }}><button onClick={() => setShowForgotPassword(true)} style={{ color: '#667eea', background: 'none', border: 'none', cursor: 'pointer', fontSize: '14px' }}>Forgot Password?</button></div>
        {showForgotPassword && (<div style={{ marginTop: '10px' }}><form onSubmit={handleForgotPassword}><input type="email" placeholder="Enter your email" value={forgotEmail} onChange={(e) => setForgotEmail(e.target.value)} className="form-input" /><button type="submit" className="login-btn" style={{ marginTop: '10px' }}>Reset Password</button></form>{forgotMessage && <p style={{ color: forgotMessage.includes('Error') ? '#e53e3e' : '#38a169', marginTop: '10px', textAlign: 'center', fontWeight: 'bold', padding: '10px', borderRadius: '5px', backgroundColor: forgotMessage.includes('Error') ? '#fce4e4' : '#e6fffa' }}>{forgotMessage}</p>}</div>)}

        <div className="error-msg">{message}</div>
        <div style={{ textAlign: 'center', marginTop: '20px' }}>{isRegistering ? (<p>Already have an account? <button onClick={() => setIsRegistering(false)} style={{ color: '#667eea', background: 'none', border: 'none', cursor: 'pointer' }}>Login</button></p>) : (<p>Don't have an account? <button onClick={() => setIsRegistering(true)} style={{ color: '#667eea', background: 'none', border: 'none', cursor: 'pointer' }}>Sign Up</button></p>)}</div>
      </div>
    </div>
  );
}
